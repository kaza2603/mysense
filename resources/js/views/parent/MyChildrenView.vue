...
const myChildren = computed(() => {
    ...
    if (!studentsStore.students || !Array.isArray(studentsStore.students))
        return [];
    ...
    return studentsStore.students.map((student) => ({
        ...
        school:
            student.school_name || student.school || "Sekolah tidak diketahui",
...
const fetchAllQuizAttempts = async (studentId) => {
    ...
    try {
        ...
        const attempts =
            await studentPerformanceStore.fetchAllQuizAttempts(studentId);
    ...
    for (const child of myChildren.value) {
        ...
        if (child.schoolId && child.classId && !child.class) {
            ...
            try {
                ...
                if (classData) {
                    console.log(
                        `Found class: ${classData.name || classData.class_name}`,
                    );
...
const currentChildStats = computed(() => {
    ...
    const stats = studentPerformanceStore.getPerformanceStats(
        selectedChildId.value,
    );
    const topicData = studentPerformanceStore.getTopicPerformance(
        selectedChildId.value,
    );
...
const weeklyActivityData = computed(() => {
    if (!selectedChildId.value) return null;
    const activity = studentPerformanceStore.getWeeklyActivityData(
        selectedChildId.value,
    );
    ...
    return {
        labels: activity.map((a) =>
            new Date(a.date).toLocaleDateString("ms-MY", { weekday: "short" }),
        ),
        datasets: [
            {
...
const topicPerformanceData = computed(() => {
    if (!selectedChildId.value) return null;
    const topics = studentPerformanceStore.getTopicPerformance(
        selectedChildId.value,
    );
    ...
    return {
        labels: completedTopics.map((t) =>
            t.topic ? t.topic.replace(/^Unit\s+/i, "") : "Topik",
        ),
        datasets: [
            {
                label: "Markah Kuiz (%)",
...
const timeDistributionData = computed(() => {
    ...
    return {
        ...
        datasets: [
            {
                data: completedTopics.map(
                    (t) => (t.completedQuizzes || 0) * 30,
                ), // Estimate 30 minutes per quiz
                backgroundColor: [
                    "#93c5fd",
                    "#86efac",
                    "#fdba74",
...
const monthlyProgressData = computed(() => {
    if (!selectedChildId.value || allQuizAttempts.value.length === 0)
        return null;
    ...
    for (let i = 5; i >= 0; i--) {
        ...
        const totalHours =
            monthAttempts.reduce(
                (acc, attempt) => acc + (attempt.time_spent || 0),
                0,
            ) / 60;
...
const learningStyleData = computed(() => {
    if (!selectedChildId.value) return null;
    const styleData = studentPerformanceStore.getLearningStylePreference(
        selectedChildId.value,
    );
...
const learningStyle = computed(() => {
    ...
    activities.forEach((activity, index) => {
        const quizType = (
            activity.quiz_type ||
            activity.type || // ← add this fallback
...
const learningAnalytics = computed(() => {
    if (!selectedChildId.value) return null;
    return studentPerformanceStore.getLearningStylePreference(
        selectedChildId.value,
    );
});
...
const dashboardStats = computed(() => {
    ...
    if (allQuizAttempts.value.length > 0) {
        totalTimeSpentMinutes = allQuizAttempts.value.reduce(
            (total, attempt) => {
    ...
    console.log("Time calculation:", {
        ...
        attemptsWithTime: allQuizAttempts.value.filter((a) => a.time_spent > 0)
            .length,
    });
...
onMounted(async () => {
    ...
    if (auth.currentUser?.email || auth.currentUser?.parent_email) {
        const parentEmail =
            auth.currentUser?.email || auth.currentUser?.parent_email;
    ...
    if (myChildren.value.length > 0) {
        ...
        if (routeStudentId) {
            ...
            targetChild = myChildren.value.find(
                (child) => child.id === routeStudentId,
            );
            if (targetChild) {
                console.log(
        ...
        try {
            ...
            await Promise.all([
                studentPerformanceStore.fetchStudentAnalytics(
                    selectedChildId.value,
                ),
            ...
            performanceError.value =
                "Gagal memuatkan data prestasi. Sila cuba lagi.";
        } finally {
...
watch(selectedChildId, async (newId) => {
    if (newId) {
        ...
        try {
            ...
            console.log(
                "Performance data and quiz attempts fetched successfully for:",
                newId,
            );
        } catch (error) {
...
watch(
    ...
    (newStudentId) => {
        if (newStudentId && myChildren.value.length > 0) {
            const targetChild = myChildren.value.find(
                (child) => child.id === newStudentId,
            );
            if (targetChild && selectedChildId.value !== newStudentId) {
                console.log(
                    "Route parameter changed, switching to student:",
                    targetChild.name,
                );
...
<template>
    <div class="p-4 sm:p-6 md:p-8 space-y-6">
        ...
        <div
            ...
            class="flex justify-center items-center p-16"
        >
            <div
        ...
        <!-- Error state -->
        <Card
            v-if="studentsStore.error && !studentsStore.loading"
        ...
        <div
            v-else-if="!selectedChildId && myChildren.length > 0"
            class="text-center p-8"
        >
            <GraduationCap class="h-16 w-16 text-gray-400 mx-auto mb-4" />
            <h3 class="text-lg font-medium mb-2">Pilih Pelajar</h3>
            <p class="text-gray-500 max-w-md mx-auto">
                Sila pilih seorang pelajar daripada senarai di atas untuk
                melihat prestasi pembelajaran mereka.
            </p>
        </div>

        <!-- Empty state - no students registered -->
        <Card
            v-else-if="myChildren.length === 0 && !studentsStore.loading"
            class="mb-8"
        >
            <CardContent class="p-8 text-center">
                ...
                <Button @click="router.push('/parent/register-student')"
                    >Daftar Pelajar Sekarang</Button
                >
            </CardContent>
        </Card>
        ...
        <div
            v-else-if="selectedChildId && !studentsStore.loading"
            class="space-y-6"
        >
            <!-- Student info card -->
            <Card class="mb-8">
                    ...
                    <TabsContent value="overview" class="space-y-6">
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        <p class="text-2xl font-bold">
                                            {{
                                                currentChildStats
                                                    ? currentChildStats.averageScore +
                                                      "%"
                                                    : "0%"
                                            }}
                                        </p>
                        ...
                        <Card class="mb-8">
                            <CardHeader>
                                ...
                                <CardDescription
                                    >Prestasi kuiz dan aktiviti pembelajaran
                                    terbaru</CardDescription
                                >
                            </CardHeader>
                            <CardContent>
                                <div
                        ...
                        <Card class="mb-8">
                            <CardHeader>
                                ...
                                <CardDescription>
                                    Aktiviti pembelajaran mingguan dalam minit
                                </CardDescription>
                            </CardHeader>
                        ...
                        <Card>
                            <CardHeader>
                                ...
                                <CardDescription>
                                    Pencapaian mengikut topik pembelajaran
                                </CardDescription>
                            </CardHeader>
                            <CardContent class="h-80">
                                <Bar
                    ...
                    <TabsContent value="progress" class="space-y-6">
                        ...
                        <Card class="mb-8">
                            <CardHeader>
                                ...
                                <CardDescription>
                                    Peningkatan prestasi sepanjang tahun
                                </CardDescription>
                            </CardHeader>
                            <CardContent class="h-80">
                                <Line
                        ...
                        <Card>
                            <CardHeader>
                                <CardTitle
                                    >Distribusi Masa Pembelajaran</CardTitle
                                >
                                <CardDescription>
                                    Pembahagian masa mengikut topik
                                </CardDescription>
                            </CardHeader>
                            <CardContent class="h-80">
                                <Doughnut
                    ...
                    <TabsContent value="analytics" class="space-y-6">
                        ...
                        <Card class="mb-8">
                            <CardHeader>
                                ...
                                <CardDescription
                                    >Gaya pembelajaran dan kekuatan anak
                        ...
                        <Card>
                            <CardHeader>
                                <CardTitle
                                    >Visualisasi Gaya Pembelajaran</CardTitle
                                >
                                <CardDescription>
                                    Kecenderungan gaya pembelajaran dalam bentuk
                                    radar chart
                                </CardDescription>
                            </CardHeader>
                            <CardContent class="h-80">
